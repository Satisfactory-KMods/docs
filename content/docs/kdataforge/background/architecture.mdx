...
```
 <your-mod>/DataForge/**/*.yml
        │  scan (FKDFPackScanner, pruned recursive walk)
        ▼
 pack.yml manifests → FKDFPack          *.yml → parallel parse (ryml, worker threads)
        │                                        │
        ▼                                        ▼
 dependency topology / ready tie-breaks  FKDFNode document trees (engine-side model)
        │                                        │
        └────────────────┬───────────────────────┘
                         ▼
              UKDFSubsystem (GameInstance)
              ├── condition evaluation (gameVersion / hasMod / modVersion / hasClass)
              ├── root-type detection (`type:` key or `<name>.<type>.yml`)
              ├── handler registry (root type → IKDFDataEditorHandler, priority-sorted)
              ▼
     Stage pipeline (fixed EKDFStage order — never reordered by handlers):
     GameTags → Assets → Localization → CDOChanges → Items → Recipes → Buildings
     → Schematics → Research → Unlocks → SubsystemMods → RuntimePatches → Validation
                         │
                         ▼
     FKDFOpEngine + FKDFValueCodec + FKDFPropertyResolver (reflection engine)
     ├── vanilla snapshots for CDO/editor writes     (FKDFVanillaCache)
     ├── op application on CDOs / data assets / matched objects
     ├── patch records (FKDFPatchRecord: report and editor history)
     └── actor patch registration → UKDFActorPatchSubsystem (per world)
```
...
`UKDFWorldModule` runs per world. It registers the `/kdf` chat command and queues content
registration into `UModContentRegistry` before that registry freezes at world begin play.
...
| ---------------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `UKDFSubsystem`           | GameInstanceSubsystem   | Owner of everything: loader, handler registry, class cache, vanilla cache, patch records, actor patches |
...
FString/FName/FText (with the `{key, ns, source}` localized form), object/class references (hard
...
special-cased so tag containers accept append/remove/clear/set with plain tag names. The full list
